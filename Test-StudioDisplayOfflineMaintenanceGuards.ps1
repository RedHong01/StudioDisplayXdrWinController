...
$results.Add((Test-FileContains -Path $manager -Pattern 'HdrGateOpened' -Name "guard:hdr-open-during-settle-invalidates-stale-gate" -Detail "transient HDR support during hot-plug settle records a fresh marker so auto repair cannot restore stale Apple USB gates")) | Out-Null
...
$results.Add((Test-FileContains -Path $autoRepair -Pattern 'persisted Apple USB reboot-required gate is still active' -Name "guard:auto-repair-persisted-3010-gate-blocks-deep-repair" -Detail "persisted Apple USB 3010 gate blocks deep repair even when the current probe is partially degraded")) | Out-Null
